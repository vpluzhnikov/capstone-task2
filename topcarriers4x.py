...
prepared_stmt = session.prepare ( "INSERT INTO " + keyspace + "." + table +
                                  " (airport, carrier, avgdelay) VALUES (?, ?, ?)")

def carrier_tuple(line):
    try:
        return ((line.split(',')[4], line.split(',')[2], 1), float(line.split(',')[9]))
    except:
        return ('error', 0)
...
if __name__ == "__main__":
    ...
    sc = SparkContext(appName="Top10Carriers4X")
    if len(sys.argv) != 2:
...
#        print(airport)
        sorted_carriers = sorted(airportsStat[airport], key=lambda x:x[1])
